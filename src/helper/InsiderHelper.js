      ...
      (type, data) => {
        switch (type) {
          case InsiderCallbackType.NOTIFICATION_OPEN:
            ...
            setTimeout(() => {
              console.log("[INSIDER][NOTIFICATION_OPEN2]: setTimeout method worked.");
              ...
              if (data?.data?.key === "voucher_birthday") {
                UI.showChildren(
                  <PopupBanner
                    title={"Birthday month"}
                    content={
                      "Med247 offers customers a 100% discount on examination fees with specialists (Applied to Pediatrics, Internal Medicine, Otolaryngology, Nutrition)"
                    }
                    btnLeftText={"See more"}
                    btnLeftAction={() =>
                      NavigationService.navigate("ListPromotion", {
                        detailCode: data?.data?.code,
                      })
                    }
                    btnRightAction={() => {
                      NavigationService.popToTop();
                      setTimeout(() => {
                        PromotionHelper.bookingWithPromotion(
                          "",
                          data?.data?.code
                        );
                      }, 100);
